...
def _manifest(index: int, *, mode="research", revision="r1") -> ChallengeManifest:
    return ChallengeManifest(
        challenge_id=f"pwn-{index}", event="private-pilot", description=f"private pwn fixture {index}",
        runner_image_digest=RUNNER, challenge_revision=revision, category_hint="pwn", benchmark_policy=mode,
    )


def _case(index: int, *, mode="research", revision="r1", category="pwn") -> BenchmarkCase:
    return case_from_manifest(
        _manifest(index, mode=mode, revision=revision), {}, case_id=f"case-{index}",
        category=category, difficulty="controlled",
    )


def _experiment(*, mode=EvaluationMode.RESEARCH, max_steps=50, seed=7) -> ExperimentContract:
    return ExperimentContract(
        mode=mode, model_id="fixed-model", model_revision="model-r1",
        controller_revision="controller-r1", tool_inventory=("argv", "session"),
        sandbox_id="sandbox-r1", oracle_policy_id="external-flag-oracle-r1",
        max_steps=max_steps, max_wall_seconds=300.0, max_tokens=100_000, seed=seed,
    )


def _research_corpus(cases):
    return freeze_corpus(
        name="private-pwn-pilot", revision="corpus-r1", mode=EvaluationMode.RESEARCH,
        cases=cases, unpublished=True,
    )


def _outcome(*, completed=False, facts=(), failures=(), tools=0, steps=0, wall=1.0):
    return RawRunOutcome(
        completed_claimed=completed, verified_fact_keys=tuple(facts), failure_signatures=tuple(failures),
        tool_calls=tools, steps=steps, wall_seconds=wall,
    )


def _adjudication(*, accepted=False, proof=None, invalid=(), evidence_sha=ADJUDICATION_SHA,
                  adjudicator_id="fixture-independent-adjudicator"):
    return IndependentAdjudication(
        adjudicator_id=adjudicator_id, evidence_sha256=evidence_sha, oracle_accepted=accepted,
        highest_proof_level=proof, invalid_verified_fact_keys=tuple(invalid),
    )


def _execution_evidence(byte="a"):
...
def test_case_identity_is_derived_from_manifest_revision_and_frozen():
    one = _case(1, revision="r1"); changed = _case(1, revision="r2")
...
def test_corpus_and_plan_require_immutable_tuple_collections():
    ...
    corpus = _research_corpus([case]); minimal, verified = paired_specs(case=case, experiment=_experiment())
    with pytest.raises(ValueError, match="immutable tuple"):
...
def test_first_pwn_pilot_qualification_requires_10_to_15_unpublished_research_pwn_cases():
    ...
    ten = _research_corpus([_case(i) for i in range(10)]); validate_first_pwn_pilot(ten)
    mixed = list(ten.cases); mixed[-1] = _case(99, category="reverse")
...
def test_ab_comparison_key_requires_same_case_model_tools_sandbox_oracle_budget_and_seed():
    case = _case(1); experiment = _experiment(); minimal, verified = paired_specs(case=case, experiment=experiment)
    assert minimal.comparison_key() == verified.comparison_key(); assert minimal.run_id() != verified.run_id()
...
def test_first_ab_plan_requires_exact_frozen_case_and_canonical_two_arms():
    case = _case(1); corpus = _research_corpus([case]); minimal, verified = paired_specs(case=case, experiment=_experiment())
    plan = BenchmarkPlan(corpus, LeakagePolicy.research(), (minimal, verified)); assert len(plan.fingerprint()) == 64
    with pytest.raises(ValueError, match="exactly two"):
        BenchmarkPlan(corpus, LeakagePolicy.research(), (minimal,))
    altered = BenchmarkRunSpec(replace(case, difficulty="different-label"), _experiment(), ArmConfig.verified())
    with pytest.raises(ValueError, match="exact frozen"):
        BenchmarkPlan(corpus, LeakagePolicy.research(), (minimal, altered))
    noncanonical = BenchmarkRunSpec(case, _experiment(), ArmConfig(BenchmarkArm.VERIFIED, True, True, True, False, True))
...
def test_independent_adjudication_and_execution_evidence_require_identity_hashes():
    ...
    with pytest.raises(ValueError, match="executor_fingerprint"):
        RunExecutionEvidence("x", "bad", "attestor", "b" * 64, "c" * 64)


def test_independent_oracle_is_success_authority_and_false_completion_is_counted():
    case = _case(1); minimal, verified = paired_specs(case=case, experiment=_experiment())
    minimal_record = build_run_record(
        minimal,
        _outcome(completed=True, failures=("same", "same", "other", "same"), tools=8, steps=12, wall=20.0),
    ...
    verified_record = build_run_record(
        verified, _outcome(completed=True, tools=5, steps=9, wall=14.0),
    ...
    assert comparison.minimal_success_rate == 0.0 and comparison.verified_success_rate == 1.0
...
def test_false_fact_metric_requires_independent_labels_to_reference_reported_facts():
    ...
    record = build_run_record(
        verified, _outcome(facts=("ctf.pwn.arch", "ctf.pwn.control_flow")),
        _adjudication(proof=ProofLevel.P1_PRIMITIVE, invalid=("ctf.pwn.control_flow",)),
        _execution_evidence(),
    )
    assert record.false_fact_count == 1
    with pytest.raises(ValueError, match="reported by the run"):
        build_run_record(
            verified, _outcome(facts=("ctf.pwn.arch",)),
            _adjudication(proof=ProofLevel.P0_SURFACE, invalid=("ctf.pwn.remote_behavior",)),
            _execution_evidence(),
        )
...
def test_research_and_competition_results_cannot_be_aggregated_or_compared():
    ...
    research_record = build_run_record(research_spec, _outcome(), _adjudication(), _execution_evidence())
    competition_spec = BenchmarkRunSpec(_case(2, mode="competition"), _experiment(mode=EvaluationMode.COMPETITION), ArmConfig.verified())
    competition_record = build_run_record(
        competition_spec, _outcome(), _adjudication(evidence_sha="3" * 64), _execution_evidence("d")
    )
...
def test_result_bundle_requires_exact_plan_complete_evidence_and_detects_tamper(tmp_path):
    ...
    records = (
        build_run_record(minimal, _outcome(), _adjudication(evidence_sha="4" * 64), _execution_evidence("a")),
        build_run_record(verified, _outcome(), _adjudication(evidence_sha="5" * 64), _execution_evidence("d")),
    )
    bundle = BenchmarkResultBundle.finalize(plan, records); path = tmp_path / "results.json"; digest = bundle.save(path)
    assert len(digest) == 64
    body = BenchmarkResultBundle.verify_saved(path, expected_plan_fingerprint=plan.fingerprint())
    ...
    with pytest.raises(IntegrityError, match="integrity mismatch"):
        BenchmarkResultBundle.verify_saved(path, expected_plan_fingerprint=plan.fingerprint())
