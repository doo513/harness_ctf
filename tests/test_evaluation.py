...
def test_research_leakage_policy_is_fail_closed():
    ...
    assert policy.exact_challenge_name_search_allowed is False
    assert policy.writeup_search_allowed is False
    assert policy.direct_flag_search_allowed is False
...
def test_ab_comparison_key_requires_same_case_model_tools_sandbox_oracle_budget_and_seed():
    ...
    changed_budget = BenchmarkRunSpec(
        case=case,
        experiment=_experiment(max_steps=51),
...
def test_first_ab_plan_requires_exact_frozen_case_and_canonical_two_arms():
    ...
    altered_case = replace(case, difficulty="different-label")
    altered = BenchmarkRunSpec(altered_case, _experiment(), ArmConfig.verified())
    with pytest.raises(ValueError, match="exact frozen"):
        BenchmarkPlan(corpus, LeakagePolicy.research(), (minimal, altered))

    noncanonical_verified = BenchmarkRunSpec(
        case,
        _experiment(),
        ArmConfig(
...
def test_false_fact_metric_requires_independent_labels_to_reference_reported_facts():
    case = _case(1)
    _, verified = paired_specs(case=case, experiment=_experiment())
...
def test_result_bundle_requires_exact_plan_runs_and_detects_tamper(tmp_path):
    ...
    verified_body = BenchmarkResultBundle.verify_saved(
        path,
...
def test_fixture_corpus_does_not_qualify_as_first_pwn_pilot_by_default():
    fixture = _research_corpus([_case(1)])
    with pytest.raises(ValueError, match="10-15"):
        validate_first_pwn_pilot(fixture)
