...
def test_crash_backend_selects_only_pre_registered_qemu_profile(tmp_path: Path) -> None:
    ...
    assert {item["role"] for item in launch["runtime"]["runtime_artifacts"]} == {
        "qemu",
        "loader",
        "sysroot",
    }
...
def test_crash_verifier_accepts_same_schema2_runtime_and_rejects_runtime_mix(tmp_path: Path) -> None:
    ...
    accepted = verifier.verify(
        candidate,
        _context_for_records(tmp_path / "same", [_schema2_record(), _schema2_record()]),
    )
    assert accepted.verified
    assert accepted.details["runtime_fingerprint"] == _schema2_record()["runtime_fingerprint"]

    rejected = verifier.verify(
        candidate,
        _context_for_records(
            tmp_path / "mixed",
            [_schema2_record(), _schema2_record(runtime_profile="different-runtime")],
        ),
    )
...
def test_crash_verifier_rejects_tampered_runtime_fingerprint(tmp_path: Path) -> None:
    ...
    result = CrashReproducibleVerifier().verify(
        candidate,
        _context_for_records(tmp_path, [first, second]),
    )
