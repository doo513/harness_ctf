...
def main()->int:
 with tempfile.TemporaryDirectory(prefix="ctf-crash-probe-") as td:
  ...
  first=json.loads(results[0].output["stdout"].strip());assert first["schema_version"]==2
  ...
  ctx={"state":{"artifacts":refs,"evidence_refs":refs,"observations":obs},"artifact_root":str(store.root),"claim_evidence_refs":refs,"claim_key":"ctf.pwn.crash_reproducible"}
  candidate={"target_sha256":target_sha,"input_sha256":input_sha,"signal":11,"runtime_fingerprint":first["runtime_fingerprint"],"launch_fingerprint":first["launch_fingerprint"]}
  v=CrashReproducibleVerifier();accepted=v.verify(candidate,ctx);assert accepted.verified,accepted.reason;assert not v.verify({**candidate,"signal":6},ctx).verified;assert not v.verify({**candidate,"runtime_fingerprint":"0"*64},ctx).verified
  one=dict(ctx);one["claim_evidence_refs"]=[refs[0]];assert not v.verify(candidate,one).verified
  print(json.dumps({"probe":"pwn-crash-semantic-live","all_passed":True,"attestation_source":att.source,"signal":11,"evidence_count":2,"target_sha256":target_sha,"input_sha256":input_sha,"runtime_fingerprint":first["runtime_fingerprint"],"launch_fingerprint":first["launch_fingerprint"]},sort_keys=True,indent=2))
 return 0
